...
class MyWindow(arcade.Window):
    ...
    # can i make a history stack here?

    # cant install arcade
    # use dict in seqlayer
    # use tuple for on_paint -> store into Bset
    # wait so i can use tuple right?
    ...
    def on_init(self):
        ...
        Complexity:
        - Worst case: O(1)
        - Best case: O(1) instantiating an object is o1
        """
        self.track_undo = UndoTracker() #O(max capacity)
        self.action_history_stack = self.track_undo.history_stack #O(1) 
        self.track_replay = ReplayTracker() #O(max capacity)
    ...
    def on_reset(self):
        ...
        """
        self.track_undo.history_stack.clear() #O(1) 
    ...
    def on_paint(self, layer: Layer, px, py):
        ...
        Raises:
        - TypeError: SequenceLayerStore
        - Exception: AdditiveLayerStore

        Returns:
        - None

        Complexity:
        - Worst case: O(nm) where n depends on the brush size & px, and m depends on the brush size & py
        - Best case: O(nm), same with worst case because no matter what, the nested for loop will happen.

        complexity = O(n) * O(m) = O(nm) where n depends on the brush size & px, and m depends on the brush size & py
        """
        add_paint_action = PaintAction()
        for x in range(px - self.grid.brush_size, px + self.grid.brush_size + 1): #O(n) where n is  range(px - self.grid.brush_size, px + self.grid.brush_size + 1)
            for y in range(py - self.grid.brush_size, py + self.grid.brush_size + 1): #O(n) * O(m) where m is range(py - self.grid.brush_size, py + self.grid.brush_size + 1)
                if abs(x - px) + abs(y - py) <= self.grid.brush_size: #O(1) * O(n) * O(m)
                    if 0 <= x < self.grid.x_size and 0 <= y < self.grid.y_size: #O(1) * O(1) * O(n) * O(m)
                        if self.grid[x][y].add(layer) == True: #O(1) * O(1) * O(1) * O(n) * O(m)

                            add_action_step = PaintStep((x,y), layer)
                            add_paint_action.add_step(add_action_step) 

        self.track_undo.add_action(add_paint_action)  
        self.track_replay.add_action(add_paint_action) 
    ...
    def on_undo(self):
        ...
        - Best case: O(1) same as worst case since the input does not affect the growth speed of the algorithm
        """
        undid_action = self.track_undo.undo(self.grid) #O(1) 
        if undid_action != None: #O(1) 
            # self.track_undo.add_action(undid_action)
            self.track_replay.add_action(undid_action, True) #O(1) 
    ...
    def on_redo(self):
        ...
        - Best case: O(1) same as worst case since the input does not affect the growth speed of the algorithm
"""
        redo_Action = self.track_undo.redo(self.grid) #O(1) 
        if redo_Action != None: #O(1) 
            self.track_replay.add_action(redo_Action) #O(1) 
...
""" 
        ...
        self.track_undo.add_action(PaintAction(is_special = True)) #O(1) 
        self.track_replay.add_action(PaintAction(is_special = True)) #O(1) 
...
"""      
    ...
    def on_replay_next_step(self) -> bool:
        ...
        - Best case: O(1) same as worst case since the input does not affect the growth speed of the algorithm
        """
        while not self.track_replay.replay_queue.is_empty(): #O(n) where n is the length of self.track_replay.replay_queue
            self.track_replay.play_next_action(self.grid) #O(n)
            return False #O(n)
        return True #O(1) 
